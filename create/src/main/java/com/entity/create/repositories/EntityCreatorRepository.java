...
public interface EntityCreatorRepository extends JpaRepository<EntityCreatorTestingEntity, Long> {
	...
	@Query(value = "select case when count(*)!=0 then true else false end from information_schema.\"columns\" where table_name= :tableName and data_type = 'json'", nativeQuery = true)
	boolean checkJson(String tableName);
